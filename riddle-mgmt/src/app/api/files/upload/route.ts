...
async function handlePresignedUpload(
  ...
  const { fileId, s3Key, storedName, originalName, fileSize, mimeType, filePassword, userId: requestedUserId, description, folder } = body;
  ...
  const finalPassword = filePassword || "creative";
  ...
  const passwordHash = bcrypt.hashSync(finalPassword, 12);
  ...
  await dbRun(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    fileId, targetUserId, storedName, originalName || "file", s3Key,
...
async function handleFormDataUpload(
  ...
  await dbRun(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [
    fileId, targetUserId, storedName, file.name, s3Key, buffer.length,
